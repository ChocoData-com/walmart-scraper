"""
Walmart Product - Chocodata Walmart Scraper API
...
Runnable example. It calls the LIVE API and prints the real JSON response.
    ...
    python walmart_scraper_api_codes/product.py
...
API = "https://api.chocodata.com/api/v1/walmart/product"
...
def product(item_id: str) -> dict:
    """Full product record (price, rating, seller, specs, breadcrumbs) by us_item_id."""
...
if __name__ == "__main__":
    data = product("18611919209")
    print(json.dumps(data, indent=2)[:2000])
    print()
    print(f"{data['general']['title'][:50]} | ${data['price']['price']} | {data['rating']['rating']} stars ({data['rating']['count']})")
