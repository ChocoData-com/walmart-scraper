"""
Walmart Related Queries - Chocodata Walmart Scraper API
...
Runnable example. It calls the LIVE API and prints the real JSON response.
    ...
    python walmart_scraper_api_codes/related_queries.py
...
API = "https://api.chocodata.com/api/v1/walmart/relatedqueries"
...
def related_queries(query: str) -> dict:
    """Walmart's own related-search suggestions for a query (keyword research)."""
...
if __name__ == "__main__":
    data = related_queries("laptop")
    print(json.dumps(data, indent=2)[:2000])
    print()
    qs = [q["query"] for q in data["related_queries"]]
    print(f"{len(qs)} related queries: {qs[:5]}")
