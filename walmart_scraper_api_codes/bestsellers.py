"""
Walmart Bestsellers - Chocodata Walmart Scraper API
...
Runnable example. It calls the LIVE API and prints the real JSON response.
    ...
    python walmart_scraper_api_codes/bestsellers.py
...
API = "https://api.chocodata.com/api/v1/walmart/bestsellers"
...
def bestsellers(category: str) -> dict:
    """Top-selling products for a Walmart category, rank-ordered."""
    params = {"api_key": KEY, "category": category}
...
if __name__ == "__main__":
    data = bestsellers("electronics")
    print(json.dumps(data, indent=2)[:2000])
    print()
    print(f"{data['results_count']} ranked | #1: {data['results'][0]['title'][:50]}")
